...
describe("percentage patterns", function()
    ...
    it("matches '15% of 24000' and captures values", function()
        local p = patterns.percentage_of
        assert.is_not_nil(p)
        local pct, val = p.match("15% of 24000")
        assert.is_not_nil(pct)
        assert.is_not_nil(val)
        assert.are_equal("15", pct)
        assert.are_equal("24000", val)
    end)

    it("matches '24000 + 15%' and captures values", function()
        local p = patterns.percentage_add
        assert.is_not_nil(p)
        local val, pct = p.match("24000 + 15%")
        assert.is_not_nil(val)
        assert.is_not_nil(pct)
        assert.are_equal("24000", val)
        assert.are_equal("15%", pct)
    ...
    it("matches '24000 - 25%' and captures values", function()
        local p = patterns.percentage_sub
        assert.is_not_nil(p)
        local val, pct = p.match("24000 - 25%")
        assert.is_not_nil(val)
        assert.is_not_nil(pct)
        assert.are_equal("24000", val)
        assert.are_equal("25%", pct)
    end)

    it("does not match '15%of 24000' without spaces", function()
        local p = patterns.percentage_of
