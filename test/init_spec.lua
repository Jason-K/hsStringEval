...
return {
    ...
    it("formats clipboard seed with prefix preserved", function()
        local instance = Formatter:init()
        helper.setClipboard("let result = 5 + 3")
        local ok = instance:formatClipboardSeed()
        assert.is_true(ok)
        assert.equal("let result = 8", helper.getClipboard())
        assert.equal("Formatted clipboard", helper.alerts[#helper.alerts])
    end)

    it("handles formatClipboardSeed with tabs and newlines", function()
        local instance = Formatter:init()
        helper.setClipboard("x=\t2*4")
        local ok = instance:formatClipboardSeed()
        assert.is_true(ok)
        assert.equal("x=\t8", helper.getClipboard())
    end)

    it("formatClipboardSeed with no whitespace processes entire string", function()
        local instance = Formatter:init()
        helper.setClipboard("10+5")
        local ok = instance:formatClipboardSeed()
        assert.is_true(ok)
        assert.equal("15", helper.getClipboard())
    end)

    it("formatClipboardSeed preserves original on side effect", function()
        local instance = Formatter:init()
        helper.setClipboard("navigate to https://example.com")
        helper.clearAlerts()
        local ok = instance:formatClipboardSeed()
        assert.is_true(ok)
        assert.equal("navigate to https://example.com", helper.getClipboard())
        assert.equal("Opened in browser", helper.alerts[#helper.alerts])
    end)

    it("formatClipboardSeed with empty clipboard returns false", function()
        local instance = Formatter:init()
        helper.setClipboard("")
        local ok = instance:formatClipboardSeed()
        assert.is_false(ok)
        assert.equal("Clipboard empty", helper.alerts[#helper.alerts])
    end)
